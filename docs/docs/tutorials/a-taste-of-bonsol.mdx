...
Congratulations! You have minted yourself some PowPow tokens. You can now use the interactive demo to mint yourself more tokens.
...
sequenceDiagram;
  ...
  participant BSC as Bonsai Smart Contract;
  participant R as Relayers;
  participant ZK as ZK Computation;
  
  SC->>BSC: Call Bonsai Smart Contract;
  BSC->>R: Emit event;
...
There are only a few more things to do before we can use this program.
