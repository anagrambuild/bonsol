...
use anagram_bonsol_schema::{
    ...
    ExecutionRequestV1, ExecutionRequestV1Args, Input, InputArgs,
};
use anyhow::Result;
...
pub async fn execute(params: ExecuteParams) -> Result<Instruction> {
    ...
    let i1 = Input::create(builder, &InputArgs {
        input_type: input_type.into(),
        input: Some(builder.create_vector(&input)),
    })
    builder.start_vector(1);
    builder.push(i1);
    let input = builder.end_vector();
